...
-- Note: supabase_vault extension must be enabled via Dashboard > Database > Extensions
-- before running this migration. It is enabled by default on new Supabase projects.
...
BEGIN
  ...
  -- Remove from vault (use Vault API to ensure proper key cleanup)
  PERFORM vault.update_secret(sid, '', '', 'deleted');
...
DECLARE
  result TEXT;
BEGIN
  -- Only allow service_role (auth.uid() IS NULL) or the user themselves
  IF auth.uid() IS NOT NULL AND auth.uid() != p_user_id THEN
    RAISE EXCEPTION 'unauthorized: cannot access another user''s secrets';
  END IF;
...
BEGIN
  -- Only allow service_role (auth.uid() IS NULL) or the user themselves
  IF auth.uid() IS NOT NULL AND auth.uid() != p_user_id THEN
    RAISE EXCEPTION 'unauthorized: cannot modify another user''s secrets';
  END IF;
